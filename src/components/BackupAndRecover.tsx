...
import { loadPrivateKey, savePrivateKey } from "../keyWallet";
...
export const BackupAndRecover: React.FC = () => {
    ...
    for (const info of Object.values(passphrases)) {
      if (info.passphraseId === passphraseId) {
        switch (info.location) {
          ...
          case "iCloud": {
           return await (loadPrivateKey(walletId ?? '', info.passphraseId) ?? "");
          }
          default:
    ...
    for (const info of Object.values(passphrases)) {
      if (info.location === location) {
        switch (location) {
          ...
          case "iCloud": {
            const passphrase = loadPrivateKey(walletId ?? "", info.passphraseId);
            return { passphraseId: info.passphraseId, passphrase: passphrase ?? "" };
          }
          default:
            throw new Error(`Unsupported backup location ${location}`);
        }
  ...
  const passphrasePersist: (
    ...
    switch (location) {
      ...
      case "iCloud": {
        savePrivateKey(walletId ?? "", passphraseId, passphrase);
        await createPassphraseInfo(passphraseId, location);
        return { passphraseId, passphrase };
      }
      default:
  ...
  const doBackupKeys = async (passphrasePersist: () => Promise<{ passphrase: string; passphraseId: string }>) => {
    ...
    try {
      // Step 1: save Driver/localStorage -> Call API BE: /api/wallets/${walletID}
      const { passphrase, passphraseId } = await passphrasePersist();
      // Step2: await fireblocksNCW.backupKeys
