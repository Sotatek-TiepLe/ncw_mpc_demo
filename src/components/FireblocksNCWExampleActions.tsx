...
export const FireblocksNCWExampleActions: React.FC = () => {
  const { keysStatus, joinExistingWalletMode, setupStatus } = useAppStore();
  ...
  return (
    <>
      {setupStatus === "not_ready" && <>{joinExistingWalletMode ? <JoinExistingWallet /> : <GenerateMPCKeys />}</>}
      {/* {setupStatus === "not_ready" && <BackupAndRecover />} */}
      {hasAKey && (
        <>
          <Assets />
          <Transactions />
