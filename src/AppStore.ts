...
export type TSetupStatus = "not_ready" | "starting" | "ready";
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        setSetupStatus: (stt: TSetupStatus) => {
          set((state) => ({ ...state, setupStatus: stt }));
        },
        assignCurrentDevice: async () => {
          ...
          try {
           ...
           const newDevices = devices?.map((v) => {
              if ( v.deviceId === deviceId) {
                 return ({...v, walletIds: [...v.walletIds, walletId]})
              } 
              return  v
            });


            set((state) => ({ ...state, walletId, devices: newDevices, assignDeviceStatus: "success" }));
          } catch (e) {
        ...
        generateNewDeviceId: async () => {
          ...
          set((state) => ({
            ...state,
            deviceId,
            devices: [...(devices ?? []), {deviceId: deviceId, walletIds: []}],
            walletId: null,
            assignDeviceStatus: "not_started",
          }));
        ...
        addDevices: (deviceId: string) => {
          const { devices } = get();

          set((state) => ({ ...state, devices: [...(devices ?? []),  {deviceId: deviceId, walletIds: []}] }));
        },
        ...
        backupKeys: async (passphrase: string, passphraseId: string) => {
          ...
          const a = await fireblocksNCW.backupKeys(passphrase, passphraseId);
          console.log(">>>backupKeys", a, JSON.stringify(a));
        },
        ...
        loginToDemoAppServer: async () => {
          ...
          try {
            ...
            set((state) => ({
              ...
              // deviceId: getOrCreateDeviceId(userId), // Not update deviceId
            }));
          } catch (e) {
        ...
        signTransaction: async (txId: string) => {
          ...
          try {
            const a = await fireblocksNCW.signTransaction(txId);
            console.log(">>> signTransaction", a);
          } catch (err: unknown) {
        ...
        generateMPCKeys: async () => {
          ...
          const a = await fireblocksNCW.generateMPCKeys(ALGORITHMS);
          console.log(">>>generateMPCKeys", JSON.stringify(a), a);
        },
    ...
    {
      name: "app",
      partialize: (state) => ({ deviceId: state.deviceId, devices: state.devices, walletId: state.walletId }),
    },
  ),
);
